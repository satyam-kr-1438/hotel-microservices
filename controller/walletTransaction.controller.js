...
const WalletTransactions = dbContext.WalletTransactions
...
exports.findAll = async (req, res) => {
    ...
    //     if (search) {
    //         data = await WalletTransactions.findAndCountAll({
    //             distinct: true,
    //             offset,
    //             limit,
    //             where: {
...
exports.create = async (req, res) => {
    try {
        if (req.body.id == undefined) {
            await WalletTransactions.create(req.body);
        } else {
            await WalletTransactions.update(req.body, { where: { id: req.body.id } })
        }
...
exports.findById = async (req, res) => {
    try {
        const { id } = req.params
        const data = await WalletTransactions.findOne({
            where: { id }
        });
...
exports.update = async (req, res) => {
    try {
        const { id } = req.params
        const [count, data] = await WalletTransactions.update(req.body, {
